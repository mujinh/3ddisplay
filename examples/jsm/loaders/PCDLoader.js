...
PCDLoader.prototype = Object.assign( Object.create( Loader.prototype ), {
	...
	parse: function ( data, url ) {
		...
		function parseHeader( data ) {
			...
			var sizeSum = 0;
			// console.log("PCDheader.fields.length",PCDheader.fields.length)
			for ( var i = 0, l = PCDheader.fields.length; i < l; i ++ ) {
		...
		if ( PCDheader.data === 'ascii' ) {
			...
			for ( var i = 0, l = lines.length; i < l; i ++ ) {
				...
				if ( offset.r !== undefined ) {
					color.push( line[offset.r]/255 , line[offset.g]/255 , line[offset.b]/255 )
				}
		...
		if ( PCDheader.data === 'binary' ) {
			...
			for ( var i = 0, row = 0; i < PCDheader.points; i ++, row += PCDheader.rowSize ) {
				...
				if ( offset.rgb !== undefined ) {
					// color.push( dataview.getFloat32( row + offset.rgb, true ) );
